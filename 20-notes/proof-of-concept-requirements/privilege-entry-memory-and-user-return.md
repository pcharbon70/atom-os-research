...
The [active generic Intel-compatible x86-64 profile](x86-64-compatibility-envelope-and-test-fixtures.md) selects ring 0 for the kernel and ring 3 for services. The [Intel system-programming study](../../30-sources/intel-2026-system-programming-documentation.md) is the primary architecture reference. Qualify entry/context sequences against the declared baseline and the processor-family errata of each physical fixture; a modern manual does not make every feature available on every CPU. Interrupt gates, syscall instructions and return instructions have different frame and stack semantics. Do not transplant the earlier xv6/RISC-V trap sequence: define GDT/IDT/TSS ownership, kernel/exception stacks and the selected entry mechanism explicitly.
