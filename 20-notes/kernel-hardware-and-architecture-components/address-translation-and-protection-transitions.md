...
| Remote execution | Kernel IPI through local APIC or stronger discovered mechanism | GIC SGI/IPI path or supported broadcast TLBI scope with explicit completion rules | Kernel/SBI IPI may notify an Kay target handler that executes and acknowledges its local fence; standard RFENCE `SBI_SUCCESS` reports transmission only, so firmware execution needs a causally ordered, request- and hart-incarnation-bound platform completion primitive |
